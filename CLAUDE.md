...
> Last scanned: 2026-03-30T00:00:00Z
> Phase: All 6 phases complete. Post-MVP polish in progress.
...
| 2026-03-30 | All 6 phases shipped; objectives/OKR legacy removed; migrations updated to 011 |

---

## Change Logging Rule

Every meaningful implementation pass must update both of these files before the work is considered complete:

- `MEMORY.md` — record what changed, the milestone/phase, and verification status
- `CLAUDE.md` — update canonical project guidance when the product story, workflow, architecture, or repo operating assumptions changed

Timestamp requirement:

- every new entry or rule update must include a concrete date or timestamp
- prefer `YYYY-MM-DD` for milestone/status entries
- use a fuller timestamp when the exact scan/update time matters

Use this rule for:
- feature work
- bug fixes
- product-direction changes
- auth/integration/workflow behavior changes

If a change is too small to alter canonical guidance, still update `MEMORY.md` and explicitly decide whether `CLAUDE.md` needs a corresponding change.
...
src/
  app/
    ...
    (app)/
      ...
      objectives/               Legacy OKR routes (removed 2026-03-30)
...
supabase/
  migrations/
    ...
    010_profile_extensions.sql  Provider settings + profile role_title/bio fields
    011_export.sql              export_token, is_portable flag on achievement_records
docs/
...
| Phase 3 | Manager Review Flow — /achievements/[id]/review, review-actions | Complete |
| Phase 4 | Proof Profile — ProofProfileCard, AchievementRecordCard, locked records UI | Complete |
| Phase 5 | Export & Portability — export_token, public proof view via /proof/[token] | Complete |
| Phase 6 | Signal Ingestion — GitHub/Linear/Jira sync to Evidence, OAuth connections | Complete |
| Post-MVP | Polish, bug fixes, real OAuth wiring, test coverage, team/org features | In Progress |
